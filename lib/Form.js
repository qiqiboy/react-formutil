...
var Form = (_temp = _class = function (_Component) {
    ...
    this.$$register = function (name, $handler, prevName) {
        ...
        var $prevRegistered = _this4.$$getRegister(prevName);

        if ($prevRegistered) {
            if ($prevRegistered === $handler) {
                prevName = $prevRegistered.$name;
                delete _this4.$$registers[prevName];
                delete $prevRegistered.$name;
            }

            if ($curRegistered !== $prevRegistered) {
                utils.objectClear(_this4.$$defaultValues, prevName);
        ...
        if (name) {
            if ($curRegistered && $curRegistered.$$reserved) {
                ...
                warning(!$curRegistered || $prevRegistered, 'The Field with a name \'' + name + '\' has been registered. You will get a copy of it\'s $fieldutil!');
            }

            _this4.$$registers[$handler.$name = name] = !$curRegistered || $prevRegistered ? $handler : $curRegistered;
        }

        _this4.creatDeepRegisters();
        _this4.$render();

        return _this4.$$registers[name];
    };

    this.$$unregister = function (name, $handler, $$reserved) {
        var $registered = _this4.$$getRegister(name);
    ...
    this.$$defaultInitialize = function () {
        _this4.$$defaultValues = _this4.$$deepParseObject(JSON.parse(JSON.stringify(_this4.props.$defaultValues)));
        _this4.$$defaultStates = _this4.$$deepParseObject(JSON.parse(JSON.stringify(_this4.props.$defaultStates)));
    };
            ...
            $$fieldChangedQueue.forEach(function (item) {
                if (item.$newValue !== item.$prevValue) {
                    if ('$newValue' in item && '$prevValue' in item) {
                        var $handler = _this4.$getField(item.name);

                        if ($handler) {
                            _this4.$getField(item.name).$$triggerChange(item);
                        }
                    }
    ...
    this.$$setStates = function () {
        ...
        utils.objectEach(_this4.$$registers, function (handler, name) {
            var data = $stateTree[name] || utils.parsePath($parsedTree, name);
    ...
    this.$reset = function ($stateTree, callback) {
        ...
        return _this4.$$setStates($stateTree, function ($state, handler) {
            return handler.$$reset($state, true);
        }, callback, true);
    };
    ...
    this.$setValues = function ($valueTree, callback) {
        Object.assign(_this4.$$defaultValues, _this4.$$deepParseObject($valueTree));
