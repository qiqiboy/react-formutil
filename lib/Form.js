        ...
        return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Form.__proto__ || Object.getPrototypeOf(Form)).call.apply(_ref, [this].concat(args))), _this), _this.$$registers = {}, _this.$$deepRegisters = {}, _this.$$defaultValues = JSON.parse(JSON.stringify(_this.props.$defaultValues)), _this.$$register = function (name, $handler, preName) {
            if (preName && $handler.$$FIELD_UUID === _this.$getField(preName).$$FIELD_UUID) {
                ...
                _this.$$fieldChangedQueue.push({
                    name: preName,
                    $preValue: _this.$formutil.$weakParams[preName]
                });
            }
            ...
            if (name && $handler.$$FIELD_UUID === _this.$getField(name).$$FIELD_UUID) {
                ...
                _this.$$fieldChangedQueue.push({
                    name: name,
                    $preValue: _this.$formutil.$weakParams[name]
                });

                _this.creatDeepRegisters();
                _this.$render();
            }
        }, _this.$$fieldChangedQueue = [], _this.$$triggerFormChange = function () {
            if (_this.$$fieldChangedQueue.length) {
                var $$fieldChangedQueue = [].concat(_toConsumableArray(_this.$$fieldChangedQueue));
                ...
                $$fieldChangedQueue.forEach(function (item) {
                    if (item.$newValue !== item.$preValue) {
                        if ('$newValue' in item && '$preValue' in item) {
                            _this.$getField(item.name).$$triggerChange(item);
                        }
                ...
                if (hasFormChanged && utils.isFunction(_this.props.$onFormChange)) {
                    _this.props.$onFormChange(_this.$formutil, $newValues, $preValues);
                }
            }
        }, _this.creatDeepRegisters = function () {
            _this.$$deepRegisters = {};
