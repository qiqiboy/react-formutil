...
var Form = (_temp = _class = function (_Component) {
    ...
    this.$$register = function (name, $handler, prevName) {
        ...
        if (name) {
            if ($curRegistered) {
                if ($curRegistered.$$reserved) {
                    ...
                    warning($curRegistered === $handler, 'The Field with a name \'' + name + '\' has been registered. You will get a copy of it\'s $fieldutil!');
                }
            } else {
                _this4.$$fieldChangedQueue.push({
                    name: name,
        ...
        return _this4.$$registers[name] || $handler;
    };
