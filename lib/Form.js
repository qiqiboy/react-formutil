...
var Form = (_temp = _class = function (_Component) {
    ...
    this.$$formValidate = function (callback) {
        ...
        if (utils.isFunction($validator)) {
            ...
            if (_this3.$shouldCancelPrevAsyncValidate) {
                _this3.$shouldCancelPrevAsyncValidate();
            }
            ...
            if (utils.isPromise(result)) {
                ...
                _this3.$shouldCancelPrevAsyncValidate = function () {
                    $isCancelAsyncValidate = true;
                };

                result.catch(function (reason) {
                    if (!$isCancelAsyncValidate) {
                        _this3.$$setFormErrors(reason);
                    }
                }).then(function () {
                    if (!$isCancelAsyncValidate) {
                ...
                _this3.$$setFormErrors(result, callback);
            }
        } else {
    ...
    this.$render = function (callback) {
        return _this3.forceUpdate(callback);
    };
