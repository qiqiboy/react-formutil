...
var Form = (_temp = _class = function (_Component) {
    ...
    this.$$register = function (name, $handler, prevName) {
        ...
        if (prevName) {
            ...
            if ($registered && $handler.$$FIELD_UUID === $registered.$$FIELD_UUID) {
                delete _this3.$$registers[prevName];
        ...
        if (name) {
            ...
            if ($registered && $registered.$$reserved) {
                ...
                _this3.$$registers[name] = $registered = $handler;
            } else {
    ...
    this.$$unregister = function (name, $handler, $$reserved) {
        if (name) {
            var $registered = _this3.$$getRegister(name);
            ...
            if (hasFormChanged) {
                if (utils.isFunction(_this3.props.$onFormChange)) {
                    ...
                    requestAnimationFrame(function () {
                        return _this3.props.$onFormChange(_this3.$formutil, $newValues, $prevValues);
                    });
                }
    ...
    this.$$formValidate = function (callback) {
        return _this3.$$formValidatePromise = new Promise(function (resolve) {
    ...
    this.$validates = function () {
            ...
            var flatter = function flatter(names) {
                names.forEach(function (name) {
                    if (Array.isArray(name)) {
                        ...
                        var handler = _this3.$getField(name);
        ...
        return _this3.$onValidates(callback);
    };

    this.$onValidates = function (callback) {
        var filedValidatePromises = Object.keys(_this3.$$registers).map(function (name) {
            return _this3.$$registers[name].$onValidate();
        });

        filedValidatePromises.push(_this3.$$formValidatePromise);
