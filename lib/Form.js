...
var Form = (_temp = _class = function (_Component) {
    ...
    this.$$registers = {};
    this.$$deepRegisters = {};
    this.$$regDuplications = {};

    this.$$checkDuplication = function () {
        var $$regDuplications = _this4.$$regDuplications;

        var hasDup = void 0;

        utils.objectEach($$regDuplications, function (_ref20, name) {
            var _ref21 = _slicedToArray(_ref20, 2),
                $curRegistered = _ref21[0],
                $handler = _ref21[1];
    ...
    this.$$register = function (name, $handler, prevName) {
        _this4.$$unregister(prevName, $handler);

        if (name) {
            var $curRegistered = _this4.$$getRegister(name);

            if ($curRegistered) {
                cancelFrame(_this4.$$duplicateTimer);

                _this4.$$regDuplications[name] = [$curRegistered, $handler];
                _this4.$$duplicateTimer = requestFrame(_this4.$$checkDuplication);
            } else {
                ...
                utils.objectClear(_this4.$$defaultValues, name);
            }

            _this4.$$registers[$handler.$name = name] = $handler;
    ...
    this.$$unregister = function (name, $handler, $$reserved) {
        if (name) {
            if (name in _this4.$$regDuplications) {
                var _$$regDuplications$na = _slicedToArray(_this4.$$regDuplications[name], 2),
                    $curRegistered = _$$regDuplications$na[0],
                    _$handler = _$$regDuplications$na[1];
                ...
                delete _this4.$$regDuplications[name];
            } else if (_this4.$$registers[name] === $handler) {
                if ($$reserved) {
    ...
    this.$$defaultInitialize = function () {
        ...
        _this4.$$defaultValues = _this4.$$deepParseObject(utils.deepClone(utils.isFunction($defaultValues) ? $defaultValues(_this4.props) || {} : $defaultValues));
        _this4.$$defaultStates = _this4.$$deepParseObject(utils.deepClone(utils.isFunction($defaultStates) ? $defaultStates(_this4.props) || {} : $defaultStates));
    };
    ...
    this.$setValues = function ($valueTree, callback) {
        _this4.$$deepParseObject(utils.deepClone($valueTree), _this4.$$defaultValues);
