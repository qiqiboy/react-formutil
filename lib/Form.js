...
var Form = (_temp2 = _class = function (_Component) {
    ...
    function Form() {
        ...
        return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = Form.__proto__ || Object.getPrototypeOf(Form)).call.apply(_ref, [this].concat(args))), _this), _this.$$registers = {}, _this.$$deepRegisters = {}, _this.$$defaultValues = JSON.parse(JSON.stringify(_this.props.$defaultValues)), _this.$$register = function (name, $handler, preName) {
            if (preName && $handler.$$FIELD_UUID === _this.$getField(preName).$$FIELD_UUID) {
                delete _this.$$registers[preName];
                utils.objectClear(_this.$$defaultValues, preName);

                utils.parsePath(_this.$$registerPreValues, preName, _this.$formutil.$weakParams[preName]);
            }

            if (name) {
                _this.$$registers[name] = $handler;

                utils.parsePath(_this.$$registerNewValues, name, $handler.$picker().$value);
            ...
            if (name || preName) {
                ...
                _this.$render(_this.$$registerCallback);
            }
        }, _this.$$unregister = function (name, $handler) {
            if (name && $handler.$$FIELD_UUID === _this.$getField(name).$$FIELD_UUID) {
                delete _this.$$registers[name];
                utils.objectClear(_this.$$defaultValues, name);

                utils.parsePath(_this.$$registerPreValues, name, _this.$formutil.$weakParams[name]);

                _this.creatDeepRigesters();
                _this.$render(_this.$$registerCallback);
            }
        }, _this.$$registerPreValues = {}, _this.$$registerNewValues = {}, _this.$$registerCallback = function () {
            clearTimeout(_this.$$registerCallbackTimer);
            _this.$$registerCallbackTimer = setTimeout(function () {
                if (typeof _this.props.$onFormChange === 'function') {
                    _this.props.$onFormChange(_this.$formutil, _this.$$registerNewValues, _this.$$registerPreValues);
                    _this.$$registerPreValues = {};
                    _this.$$registerNewValues = {};
                }
            });
            ...
            var $newValues = _this.$$registerNewValues;
            var $preValues = _this.$$registerPreValues;
            var callbackQueue = [];
            ...
            _this.$render(function () {
                ...
                if (Object.keys($newValues).length) {
                    callbackQueue.push({
                        callback: _this.$$registerCallback,
                        args: []
                    });
                }
